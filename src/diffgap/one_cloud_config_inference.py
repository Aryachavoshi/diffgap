...
def compare_methods_fixed(
    ...
    *,
    ...
    """
    Pixel metrics:
      - RMSE, R² in *physical* space (e.g., Kelvin) via `custom_minmax` + `norm`
      - PSNR, SSIM in *normalized* space (truth / pred_full), with data_range from `norm`

    UHI metrics are computed on FULL frames (unmasked), in physical space
    via `uhi_phys_minmax`, independent of `region`.
    """
    from diffgap.utils import apply_region

    # -------- data_range for PSNR/SSIM in *normalized* space --------
    if norm == "0_1":
        data_range = 1.0
    elif norm in ("-1_1", "neg1_1"):
        data_range = 2.0
    else:
        # fallback: infer from truth in normalized space
        tr = truth.detach()
    ...
    # -------- Prepare full-frame truth for UHI (physical space) --------
    if urban_mask is not None:
        ...
        # counts per image; avoid div-by-zero
        ...
        truth_uhi = None
        um = None  # just to keep linters happy

    # -------- Loop over methods --------
    for name, pred_full in predictions.items():
        # ----- region selection in *normalized* space -----
        ...
        # ----- Unnormalize ONLY for RMSE/R² (physical space) -----
        ...
        # ---------- PSNR in normalized space ----------
        def _psnr(a, b):
            mse = torch.mean((a - b) ** 2)
            if mse <= 1e-20:
                return torch.tensor(float("inf"), device=a.device)
            return 20.0 * torch.log10(
                torch.tensor(data_range, device=a.device) / torch.sqrt(mse)
            )

        per_psnr = torch.stack(
            [_psnr(x_reg[i:i+1], y_reg[i:i+1]) for i in range(x_reg.size(0))]
        ).squeeze()

        # ---------- SSIM in normalized space ----------
        per_ssim = torch.stack(
            [ssim_metric(x_reg[i:i+1], y_reg[i:i+1]) for i in range(x_reg.size(0))]
        ).squeeze()
        ...
        # ---------- UHI metrics on FULL frames (physical) ----------
        if truth_uhi is not None:
            ...
            if uhi_mode == "pct":
                ...
                denom = torch.clamp(
                    torch.abs(pred_uhi) + torch.abs(truth_uhi),
                    min=2 * uhi_min_denom_k,
                )
